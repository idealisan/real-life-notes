...
- **方案**：
  ...
  - **预览下移并改为上下对照**：编辑区在上、预览在下，编辑器视图占满视口高度（`height: calc(100vh - 56px)`），上下各半同时可见（各 pane 内部滚动），不用左右对照也不用滚出页面；小屏（≤760px）自动退回普通滚动布局。
...
- 测试：`test-admin-layout.js`（旧类不再出现、导航位于顶栏、`.editor-screen` 上下对照、预览在编辑框下、三个菜单展开/插入/收起、emoji 网格、顶栏导航切换）。
