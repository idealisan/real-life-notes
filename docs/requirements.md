...
- **第三批（底部 Tab 悬浮化 + 全选入口 + 分类新建收折 + 子页空白修复）**：
  - **底部 Tab 栏改 iOS 26 悬浮胶囊**：原 `.side-nav` 通栏浅灰（`--bg-page`）贴在页面底部、与页面背景几乎同色难分辨。改为左右全圆角的独立气泡长条：`left:50%; transform:translateX(-50%)` 居中、`width:min(calc(100% - 24px), 440px)`、`border-radius:999px`、`bottom:calc(6px + var(--safe-bottom))` 与浏览器工具条留 6px 间隙、背景用 `--bg-card`（与页面 `--bg-page` 区分开）、加 `0.5px` 边框与投影，视觉上"悬浮、独立"；桌面端媒体查询重置回顶栏内居中原状。
  - **全选/反选移入文章页标题栏**：列表区批量条里的「全选」复选框移除（移动端），改为标题栏「筛选」按钮左侧新增一个全选图标按钮（`icon('checklist')`）：未选→全选当前列表；已全选→全部取消（title 栏按钮高亮 `.ios-icon-btn-on`）。批量操作条只保留「已选 N 篇 + 批量发布/存草稿/移动/删除/取消选择」，未选择时不显示。桌面端批量条保留原「全选」复选框。
  - **分类页新建表单默认隐藏**：`新建分类` 表单不再一进来就摊在列表上方，默认 `hidden`，点右上角「＋」展开（再点收起），展开后自动聚焦第一个输入框。
...
- **小屏视口自适应（iPhone SE 320px 无横向滚动）**：
  ...
  - **代码块防误伤**：`body { word-break: break-word }` 是继承属性，会继承进 `pre`/`code`，把长代码行的最小内容宽度压成容器宽（长行可能被裁且无法横向滚动）。修复：`pre, pre code { white-space: pre; word-break: normal; overflow-wrap: normal; }` 复位，长代码行保持原样溢出，交给 hljs 的 `pre code.hljs { overflow-x: auto }` 横向滚动查看（实测 code.scrollLeft 可滚）。
