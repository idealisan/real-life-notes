...
- **方案**：
  ...
  - **预览下移**：`.editor-grid` 由 `1fr 1fr` 改为单列，正文 textarea 全宽（min-height 480px），`.preview-pane` 放到编辑框下方，滚动即可见。
...
- 测试：`test-admin-layout.js`（旧类不再出现、导航位于顶栏、预览在编辑框下、三个菜单展开/插入/收起、emoji 网格、顶栏导航切换）。
