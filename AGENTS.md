...
## 实现要点（已验证，勿回退）
- **token 表单**：`type="password"` + `autocomplete="current-password"` + 稳定唯一 `id`/`name` + 正常 `<form action="./" method="get">`+submit（**必须带 `action`**）；表单内加一个 `visually-hidden` 的 `autocomplete="username"` 字段，提交时由 JS 填仓库 owner。不设 `maxlength`；**不得**写入 localStorage/IndexedDB（内存持有，刷新重连）。
  - **已放弃「浏览器自动保存 Token」**（详见 `docs/experiment-login-save-password.md`，勿再重复尝试）：纯静态零后端无法构造浏览器认可的"登录提交"，`navigator.credentials.store`/`PasswordCredential` 正被 Web 标准移除（实验特性、非 Baseline）。界面文案引导用户**手动**在浏览器密码管理器添加（网址 / GitHub 用户名 / Token），连接流程不变。`admin/login.html` 独立登录页保留（GET 提交 + sessionStorage `adminToken`/`adminRepo` + 后台 `boot()` 自动连接），不再宣称会弹保存框。
  - **Token 加密存储（推荐主方案）**：后台设置页可把当前 Token 用用户自定义密码 AES-256-GCM 加密后存入仓库 `content/.admin-token`（密文结构 `{v,kdf,iter,algo,salt,iv,ct}`，PBKDF2-SHA256 12 万次派生密钥，Web Crypto `crypto.subtle`）。之后打开后台，`boot()` 先本地 `fetch('../content/.admin-token')`、再退回 `gh.getContentPublic` 探测到密文即显示"输入密码解锁"模式（`unlockForm`，可切换手动 Token 模式），解密后自动连接。密码只在内存，不落盘。**仅 HTTPS 可用**（`crypto.subtle` 需要安全上下文）；清空密码前先「清除已保存的加密 Token」。
