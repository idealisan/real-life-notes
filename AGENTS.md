...
- 第三方库**本地托管**于 `assets/vendor/`（marked 4.3.0、DOMPurify 3.0.6、KaTeX 0.16.11 + 字体），不依赖 CDN。
...
- **渲染管线（`md.render`）**：先保护代码块（`@@CODE-n@@`）→ 提取 `$...$`/`$$...$$`（`@@MATH-n@@`，货币 `$数字` 启发式跳过）→ marked → DOMPurify（`FORBID_ATTR:['style']`）→ 回填 KaTeX HTML。
...
- 语法检查与集成测试用 node（本机未装，可临时下载到 `/tmp/opencode/node`）：
  - `node --check <file>` 语法。
  - jsdom 集成测试在 `/tmp/opencode/jstest/`：`test-site`（列表/路由）、`test-site2`（过滤/搜索/详情/公式/404）、`test-admin`（连接/发布）、`test-admin2`（草稿/编辑/删除/409 重试）。
