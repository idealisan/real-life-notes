...
2. **详情页顶部标签点击无效果**：正文阅读（`post.html?p=…`）时点击详情头部的标签（`.detail-head` 的 `.tag`），页面无任何变化，网络请求中只有对当前页面地址的请求，未出现 `index.html?q=…` 的跳转。**排查结论与修复**：真正的根因是**详情页顶部分类导航**（`.cat-nav`）——`catLink()` 的 `onClick` 无条件 `preventDefault()` + `render()`，而 `render()` 在 `MODE==='post'` 时只会重渲染当前详情页，导致点击顶部导航/标签类链接毫无变化。已修复：`catLink()` 与 `tagLink()` 在 `MODE==='post'` 分支改为 `window.location.href` 显式跳转（分类→`index.html?cat=`、标签→`index.html?q=`）；列表页仍原地渲染。测试：`test-post.js` 断言点击分类导航/标签后 `defaultPrevented=true`（导航接管）。
